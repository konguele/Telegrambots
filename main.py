...
def command_long_text(m):
    ...
    time.sleep(3)
    server = (m.text[len('/cpu'):])
    subprocess.call(["robocop", "--robocop", "--cpu", "--server", server])
...
def command_long_text(m):
    cid = m.chat.id
    bot.send_message(cid, "Recogiendo información de CPU del servidor " + m.text[len("/exec"):])
    bot.send_chat_action(cid, 'typing')
    time.sleep(3)
    f = "m.text[len('/exec'):]"
    os.system("robocop --robocop --cpu --server (f)")
